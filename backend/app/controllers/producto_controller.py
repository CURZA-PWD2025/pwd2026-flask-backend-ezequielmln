...
class ProductoController (Controller):
    ...
    def create(request) -> tuple[Response, int]:
        ...
        precio_costo:float = request['precio_costo']
        precio_venta:float = request['precio_venta']
        stock_actual:float = request['stock_actual']
        stock_minimo:float = request['stock_minimo']
        ...
        if error is None:
            try:
                ...
                db.session.rollback()
                return jsonify({'mensaje': "Producto ya registrado"}), 409
        ...
        if error is None:
            ...
            if producto:
                try:
                    ...
                    error = 'el producto ya existente' 
